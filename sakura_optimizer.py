        ...
        with urllib.request.urlopen(current_url, timeout=5) as response:
            ...
            if must_update:
                ...
                with open(__file__, "w", encoding="utf-8") as f:
                    f.write(new_code)
                print(f"{_c('92;1')}Update complete! Reloading Sakura Optimizer...{_c('0')}")
...
def main():
    ...
    os_name = detect_os()
    backup = Path(__file__ + ".bak")
    if backup.exists() and not args.json:
        print(f"{_c('96;1')}Previous update backup found at {backup}{_c('0')}")
